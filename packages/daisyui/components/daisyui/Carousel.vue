...
function _toggleCarousel(event: MouseEvent, index: number) {
  event.preventDefault()
  const carousel = carouselRef.value!
  const target = carousel.querySelectorAll('.carousel-item')[index - 1] as HTMLDivElement
...
<template>
  <div ref="carouselRef" class="carousel rounded-box">
